...
def edit_profile(request):
    # 獲取或創建使用者的 UserProfile
    user_profile, created = UserProfile.objects.get_or_create(user=request.user)

    if request.method == 'POST':
        # 在 POST 請求中處理表單數據
        # request.FILES 用於處理檔案上傳 (例如頭像)
        form = UserProfileForm(request.POST, request.FILES, instance=user_profile)
        if form.is_valid():
            # 在保存表單之前，可能需要處理 email 字段（如果 UserProfileForm 中有 email 字段）
            # 或者單獨更新 User 模型的 email 字段
            user = request.user
            new_email = form.cleaned_data.get('email') # 從UserProfileForm獲取email
            if new_email and user.email != new_email:
                user.email = new_email
                user.save()

            form.save() # 保存 UserProfile 的更改
            messages.success(request, '個人資料已成功更新！')
            return redirect('profile') # 重定向回個人檔案顯示頁面
        else:
            messages.error(request, '更新個人資料失敗，請檢查輸入。')
    else:
        # 在 GET 請求中顯示當前資料的表單
        # 如果您的 UserProfileForm 中包含了 email 字段，並且它的值需要從 User 模型中初始化
        # 確保您的 UserProfileForm 在 __init__ 中處理了這個邏輯
        form = UserProfileForm(instance=user_profile)
        # 手動為 email 字段設定初始值，因為它在 User 模型上
        form.fields['email'].initial = request.user.email

    context = {
        'form': form,
        'user': request.user # 將 user 物件傳遞給模板，以便顯示用戶名等
    }
